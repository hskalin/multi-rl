...
class VectorizedReplayBuffer:
    def __init__(self, obs_shape, action_shape, capacity, device):
        ...
        self.dones = torch.empty((capacity, 1), dtype=torch.int8, device=self.device)
    ...
    def add(self, obs, action, reward, next_obs, done):
        ...
        if remaining_capacity < num_observations:
            ...
            self.rewards[0:overflow] = reward[-overflow:, None]
            self.next_obses[0:overflow] = next_obs[-overflow:]
            self.dones[0:overflow] = done[-overflow:, None]
            self.full = True
        ...
        self.dones[self.idx : self.idx + remaining_capacity] = done[
            :remaining_capacity, None
        ]
