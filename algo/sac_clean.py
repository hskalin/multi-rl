...
class SAC_agent:
    def __init__(self, args):
        ...
        # )
        self.rb = VectorizedReplayBuffer(
            self.env.obs_space.shape,
            self.env.act_space.shape,
            args.buffer_size,
            self.device,
        )
    ...
    def train(self):
        ...
        for global_step in range(self.args.total_timesteps):
            ...
            self.rb.add(obs, actions, rewards, real_next_obs, dones)
            ...
            if global_step > self.args.learning_starts:
                with torch.no_grad():
                    obs_s, action_s, reward_s, next_obs_s, done_s = self.rb.sample(
                        self.args.batch_size
                    )
                    next_state_actions, next_state_log_pi, _ = self.actor.get_action(
                        next_obs_s
                    )
                    qf1_next_target = self.qf1_target(next_obs_s, next_state_actions)
                    qf2_next_target = self.qf2_target(next_obs_s, next_state_actions)
                    min_qf_next_target = (
                        torch.min(qf1_next_target, qf2_next_target)
                        - self.alpha * next_state_log_pi
                    )

                    next_q_value = reward_s.flatten() + (
                        1 - done_s.flatten()
                ...
                qf1_a_values = self.qf1(obs_s, action_s).view(-1)
                qf2_a_values = self.qf2(obs_s, action_s).view(-1)
                ...
                if (
                    ...
                    for _ in range(
                        ...
                        pi, log_pi, _ = self.actor.get_action(obs_s)
                        qf1_pi = self.qf1(obs_s, pi)
                        qf2_pi = self.qf2(obs_s, pi)
                        ...
                        if self.args.autotune:
                            with torch.no_grad():
                                _, log_pi, _ = self.actor.get_action(obs_s)
                            alpha_loss = (
                                -self.log_alpha
