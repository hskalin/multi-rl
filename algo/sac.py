...
class SAC_Agent:
    def __init__(self, args, params):
        ...
        self.base_init(base_name, args, config)
        ...
        self.max_env_steps = config.get(
            "max_env_steps", 1000
        )  # temporary, in future we will use other approach
        ...
        self.target_entropy = (
            self.target_entropy_coef * -self.env_info["action_space"].shape[0]
        )
        ...
        self.max_episodes = (
            torch.ones(self.num_actors, device=self._device)
    ...
    def base_init(self, base_name, args, config):
        ...
        if self.env_info is None:
            # self.vec_env = vecenv.create_vec_env(
            #     self.env_name, self.num_actors, **self.env_config
            # )
            # self.env_info = self.vec_env.get_env_info()
            self.vec_env = env_map[args.env_id](args)
            self.env_info = {
                "action_space": self.vec_env.act_space,
                "observation_space": self.vec_env.obs_space,
            }
        ...
        if full_experiment_name:
            print(
                f"Exact experiment name requested from command line: {full_experiment_name}"
            )
        ...
        self.writer = SummaryWriter(
            "runs/" + config["name"] + datetime.now().strftime("_%d-%H-%M-%S")
        )
        print(
    ...
    def env_step(self, actions):
        ...
        obs, rewards, dones, episodeLen, episodeRet, truncated = (
            self.vec_env.obs_buf,
            self.vec_env.reward_buf,
            self.vec_env.reset_buf.clone(),
            self.vec_env.progress_buf.clone(),
            self.vec_env.return_buf.clone(),
            self.vec_env.truncated_buf.clone(),
        )
        self.vec_env.reset()
    ...
    def env_reset(self):
        with torch.no_grad():
            obs = self.vec_env.obs_buf.clone()
    ...
    def extract_actor_stats(
        self, actor_losses, entropies, alphas, alpha_losses, actor_loss_info
    ):
    ...
    def play_steps(self, random_exploration=False):
        ...
        for s in range(self.num_steps_per_episode):
            ...
            if random_exploration:
                ...
                with torch.no_grad():
                    action = self.act(
                        obs.float(), self.env_info["action_space"].shape, sample=True
                    )
            ...
            if not random_exploration:
                ...
                actor_loss_info, critic1_loss, critic2_loss = self.update(
                    self.epoch_num
                )
                update_time_end = time.time()
                update_time = update_time_end - update_time_start

                self.extract_actor_stats(
                    actor_losses, entropies, alphas, alpha_losses, actor_loss_info
                )
    ...
    def train(self):
        ...
        while True:
            ...
            self.writer.add_scalar(
                "performance/step_inference_rl_update_fps", fps_total, self.frame
            )
            self.writer.add_scalar(
                "performance/step_inference_fps", fps_step_inference, self.frame
            )
            self.writer.add_scalar("performance/step_fps", fps_step, self.frame)
            self.writer.add_scalar(
                "performance/rl_update_time", update_time, self.frame
            )
            self.writer.add_scalar(
                "performance/step_inference_time", play_time, self.frame
            )
            ...
            if self.epoch_num >= self.num_warmup_steps:
                self.writer.add_scalar(
                    "losses/a_loss",
                    torch_ext.mean_list(actor_losses).item(),
                    self.frame,
                )
                self.writer.add_scalar(
                    "losses/c1_loss",
                    torch_ext.mean_list(critic1_losses).item(),
                    self.frame,
                )
                self.writer.add_scalar(
                    "losses/c2_loss",
                    torch_ext.mean_list(critic2_losses).item(),
                    self.frame,
                )
                self.writer.add_scalar(
                    "losses/entropy", torch_ext.mean_list(entropies).item(), self.frame
                )

                if alpha_losses[0] is not None:
                    self.writer.add_scalar(
                        "losses/alpha_loss",
                        torch_ext.mean_list(alpha_losses).item(),
                        self.frame,
                    )
                self.writer.add_scalar(
                    "info/alpha", torch_ext.mean_list(alphas).item(), self.frame
                )
            ...
            if self.game_rewards.current_size > 0:
                ...
                if (
                    mean_rewards > self.last_mean_rewards
